...
window.__getFeweeklyClearlyResults = function () {
    ...
    // debug
    FeweeklyClearly.log = function (message) {
    ...
    // text length,
    FeweeklyClearly.getTextLength = function (text) {
        return text.replace(/[\s\n\r]+/gi, '').length;
    };

    // word count,
    FeweeklyClearly.getWordCount = function (text) {
        var _text = text;
        // do stuff
    ...
    // content
    FeweeklyClearly.getContent = function () {
        this.detectLanguage();
        return this.find();
    };

    // Detect language by randomly selected paragraphs from page
    FeweeklyClearly.detectLanguage = function () {
        ...
        // add
        for (var i = 0; i < 5; i++) {
            text += ' ' + $(_ps[Math.floor(Math.random() * _ps.length)]).text();
        }
        for (i = 0; i < 5; i++) {
            text += ' ' + $(_ds[Math.floor(Math.random() * _ds.length)]).text();
        }

        // check
        switch (true) {
        case (text.match(/([\u3000])/gi) !== null):
        case (text.match(/([\u3001])/gi) !== null):
        case (text.match(/([\u3002])/gi) !== null):
        case (text.match(/([\u301C])/gi) !== null):
            this.language = 'cjk';
    ...
    FeweeklyClearly.getAbsoluteUrl = function (url) {
    ...
    // _return__links.push({href: FeweeklyClearly.getAbsoluteUrl(_href), text: _node.textContent});
    // _return__images.push({type: 'large', src: FeweeklyClearly.getAbsoluteUrl(_node.src), width: _width, height: _height});
    // _return__images.push({type: 'medium', src: FeweeklyClearly.getAbsoluteUrl(_node.src), width: _width, height: _height});

    // Decide tagName of a node, we just need #text, a, img nodes
    FeweeklyClearly.getContent_detectNodeTagName = function (_node) {
        var _tag_name;
    ...
    FeweeklyClearly.exploreNodeAndGetStuff = function (_nodeToExplore, _justExploring) {
        ...
        var _recursive = function (_node) {
            ...
            var _tag_name = FeweeklyClearly.getContent_detectNodeTagName(_node),
                _result = {
            ...
            case ((_tag_name === '#text')):
                ...
                // result
                _result._length__plain_text = FeweeklyClearly.getTextLength(_nodeText);
                _result._count__plain_words = FeweeklyClearly.getWordCount(_nodeText);
            ...
            case (_tag_name === 'img'):
                ...
                if (_node.src && _node.src.indexOf) {
                    for (i = 0, _i = skipStuffFromDomain__images.length; i < _i; i++) {
            ...
            // child nodes
            for (i = 0, _i = _node.childNodes.length; i < _i; i++) {
            ...
            if (_result._is__container || ((_result.__index === 1) && (_justExploring === true))) {
                ...
                if (!_justExploring) {
                    switch (true) {
                    case ((FeweeklyClearly.language !== 'cjk') && ((_result._count__links * 2) >= _result._count__plain_words)):
                        /* link ratio */
                    case ((FeweeklyClearly.language !== 'cjk') && (_result._length__plain_text < (65 / 3))):
                        /* text length */
                    case ((FeweeklyClearly.language !== 'cjk') && (_result._count__plain_words < 5)):
                        /* words */
                    case ((FeweeklyClearly.language === 'cjk') && (_result._length__plain_text < 10)):
                        /* text length */
                    case ((FeweeklyClearly.language === 'cjk') && (_result._count__plain_words < 2)):
                        /* words */
                        break;
                    default:
                        // good candidate
    ...
    FeweeklyClearly.processCandidates = function (_candidatesToProcess) {
        // process this var
        ...
        for (var i = 0, _i = _candidates.length; i < _i; i++) {
            ...
            // candidate details
            _candidates[i]['__candidate_details'] = FeweeklyClearly.computeDetailsForCandidate(_candidates[i], _main);
            ...
            // points
            _candidates[i].__points_history = FeweeklyClearly.computePointsForCandidate(_candidates[i], _main);
    ...
    FeweeklyClearly.computeDetailsForCandidate = function (_e, _main) {
        var _r = {};
        ...
        var _above_text = ((0 + (_e._length__above_plain_text * 1) + (_e._length__above_plain_text / _divide__candidates)) / 2);
        var _above_words = ((0 + (_e._count__above_plain_words * 1) + (_e._count__above_plain_words / _divide__candidates)) / 2);
    ...
    FeweeklyClearly.computePointsForCandidate = function (_e, _main) {
        ...
        // total text
        this.computePointsForCandidate__do(0.10, 2, (1 - (1 - _details._ratio__length__plain_text_to_total_plain_text)), _points_history);
        this.computePointsForCandidate__do(0.10, 2, (1 - (1 - _details._ratio__count__plain_words_to_total_plain_words)), _points_history);
        if (_really_big) {
            this.computePointsForCandidate__do(0.10, 4, (1 - (1 - _details._ratio__length__plain_text_to_total_plain_text)), _points_history);
            this.computePointsForCandidate__do(0.10, 4, (1 - (1 - _details._ratio__count__plain_words_to_total_plain_words)), _points_history);
        }

        // text above
        this.computePointsForCandidate__do(0.10, 5, (1 - _details._ratio__length__above_plain_text_to_total_plain_text), _points_history);
        this.computePointsForCandidate__do(0.10, 5, (1 - _details._ratio__count__above_plain_words_to_total_plain_words), _points_history);
        if (_really_big) {
            this.computePointsForCandidate__do(0.10, 10, (1 - _details._ratio__length__above_plain_text_to_total_plain_text), _points_history);
            this.computePointsForCandidate__do(0.10, 10, (1 - _details._ratio__count__above_plain_words_to_total_plain_words), _points_history);
        }

        // links outer
        this.computePointsForCandidate__do(0.75, 1, (1 - _details._ratio__length__links_text_to_total_links_text), _points_history);
        this.computePointsForCandidate__do(0.75, 1, (1 - _details._ratio__count__links_words_to_total_links_words), _points_history);
        this.computePointsForCandidate__do(0.75, 1, (1 - _details._ratio__count__links_to_total_links), _points_history);

        // links inner
        var __lr = (this.language === 'cjk' ? 0.75 : 0.50);
        this.computePointsForCandidate__do(__lr, 1, (1 - _details._ratio__length__links_text_to_plain_text), _points_history);
        this.computePointsForCandidate__do(__lr, 1, (1 - _details._ratio__count__links_words_to_plain_words), _points_history);
        this.computePointsForCandidate__do(__lr, 1, (1 - _details._ratio__length__links_text_to_all_text), _points_history);
        this.computePointsForCandidate__do(__lr, 1, (1 - _details._ratio__count__links_words_to_all_words), _points_history);
        this.computePointsForCandidate__do(__lr, 1, (1 - _details._ratio__count__links_to_plain_words), _points_history);

        // candidates, pieces
        this.computePointsForCandidate__do(0.75, 1, (1 - _details._ratio__count__candidates_to_total_candidates), _points_history);
        this.computePointsForCandidate__do(0.75, 1, (1 - _details._ratio__count__containers_to_total_containers), _points_history);
        this.computePointsForCandidate__do(0.75, 1, (1 - _details._ratio__count__pieces_to_total_pieces), _points_history);

        // return -- will get [0] as the actual final points
        return _points_history;
    };

    FeweeklyClearly.processCandidatesSecond = function (_processedCandidates) {
        ...
        _candidates = $.map(_candidates, function (_element, _index) {
            switch (true) {
            case (_index <= 0):
            case ($.contains(_main.__node, _element.__node) === false):
                return null;
            default:
                return _element;
            }
        ...
        for (var i = 0, _i = _candidates.length; i < _i; i++) {
            ...
            // candidate details
            _candidates[i]['__candidate_details_second'] = this.computeDetailsForCandidateSecond(_candidates[i], _main);
            // check some more

            // points
            _candidates[i].__points_history_second = this.computePointsForCandidateSecond(_candidates[i], _main);
    ...
    FeweeklyClearly.computeDetailsForCandidateSecond = function (_e, _main) {
        var _r = {};
    ...
    FeweeklyClearly.computePointsForCandidateSecond = function (_e) {
        var _details = _e.__candidate_details,
        ...
        // total text
        this.computePointsForCandidate__do(0.50, 1, (1 - (1 - _details_second._ratio__length__plain_text_to_total_plain_text)), _points_history);
        this.computePointsForCandidate__do(0.50, 1, (1 - (1 - _details_second._ratio__count__plain_words_to_total_plain_words)), _points_history);

        // text above
        var __ar = (this.language === 'cjk' ? 0.50 : 0.10);
        this.computePointsForCandidate__do(__ar, 1, (1 - _details_second._ratio__length__above_plain_text_to_total_plain_text), _points_history);
        this.computePointsForCandidate__do(__ar, 1, (1 - _details_second._ratio__count__above_plain_words_to_total_plain_words), _points_history);
        this.computePointsForCandidate__do(__ar, 1, (1 - _details_second._ratio__length__above_plain_text_to_plain_text), _points_history);
        this.computePointsForCandidate__do(__ar, 1, (1 - _details_second._ratio__count__above_plain_words_to_plain_words), _points_history);

        // links outer
        this.computePointsForCandidate__do(0.75, 1, (1 - _details_second._ratio__count__links_to_total_links), _points_history);
        this.computePointsForCandidate__do(0.75, 1, (1 - _details_second._ratio__length__links_text_to_total_links_text), _points_history);
        this.computePointsForCandidate__do(0.75, 1, (1 - _details_second._ratio__count__links_words_to_total_links_words), _points_history);

        // links inner
        var __lr = (this.language === 'cjk' ? 0.75 : 0.50);
        this.computePointsForCandidate__do(__lr, 1, (1 - _details._ratio__length__links_text_to_plain_text), _points_history);
        this.computePointsForCandidate__do(__lr, 1, (1 - _details._ratio__count__links_words_to_plain_words), _points_history);
        this.computePointsForCandidate__do(__lr, 1, (1 - _details_second._ratio__length__links_text_to_all_text), _points_history);
        this.computePointsForCandidate__do(__lr, 1, (1 - _details_second._ratio__count__links_words_to_all_words), _points_history);
        this.computePointsForCandidate__do(__lr, 1, (1 - _details_second._ratio__count__links_to_plain_words), _points_history);

        // candidates, containers, pieces
        this.computePointsForCandidate__do(0.10, 2, (1 - _details_second._ratio__count__candidates_to_total_candidates), _points_history);
        this.computePointsForCandidate__do(0.10, 2, (1 - _details_second._ratio__count__containers_to_total_containers), _points_history);
        this.computePointsForCandidate__do(0.10, 2, (1 - _details_second._ratio__count__pieces_to_total_pieces), _points_history);

        // return -- will get [0] as the actual final points
        return _points_history;
    };

    FeweeklyClearly.computePointsForCandidateThird = function (_e) {
        var _details = _e.__candidate_details,
            _details_second = _e.__candidate_details_second,
            _points_history = [];
        ...
        // total text
        this.computePointsForCandidate__do(0.75, 1, (1 - (1 - _details_second._ratio__length__plain_text_to_total_plain_text)), _points_history);
        this.computePointsForCandidate__do(0.75, 1, (1 - (1 - _details_second._ratio__count__plain_words_to_total_plain_words)), _points_history);

        // text above
        this.computePointsForCandidate__do(0.50, 1, (1 - _details._ratio__length__above_plain_text_to_total_plain_text), _points_history);
        this.computePointsForCandidate__do(0.50, 1, (1 - _details._ratio__count__above_plain_words_to_total_plain_words), _points_history);
        this.computePointsForCandidate__do(0.10, 1, (1 - _details_second._ratio__length__above_plain_text_to_total_plain_text), _points_history);
        this.computePointsForCandidate__do(0.10, 1, (1 - _details_second._ratio__count__above_plain_words_to_total_plain_words), _points_history);
        this.computePointsForCandidate__do(0.10, 1, (1 - _details_second._ratio__length__above_plain_text_to_plain_text), _points_history);
        this.computePointsForCandidate__do(0.10, 1, (1 - _details_second._ratio__count__above_plain_words_to_plain_words), _points_history);

        // links inner
        this.computePointsForCandidate__do(0.50, 1, (1 - _details._ratio__length__links_text_to_all_text), _points_history);
        this.computePointsForCandidate__do(0.50, 1, (1 - _details._ratio__count__links_words_to_all_words), _points_history);
        this.computePointsForCandidate__do(0.50, 1, (1 - _details._ratio__length__links_text_to_plain_text), _points_history);
        this.computePointsForCandidate__do(0.50, 1, (1 - _details._ratio__count__links_words_to_plain_words), _points_history);
        this.computePointsForCandidate__do(0.50, 1, (1 - _details._ratio__count__links_to_plain_words), _points_history);

        // candidates, pieces
        this.computePointsForCandidate__do(0.50, 1, (1 - _details._ratio__count__candidates_to_total_candidates), _points_history);
        this.computePointsForCandidate__do(0.50, 1, (1 - _details._ratio__count__containers_to_total_containers), _points_history);
        this.computePointsForCandidate__do(0.50, 1, (1 - _details._ratio__count__pieces_to_total_pieces), _points_history);

        // return -- will get [0] as the actual final points
        return _points_history;
    };

    FeweeklyClearly.computePointsForCandidate__do = function (_ratio_remaining, _power, _ratio, _points_history) {
    ...
    FeweeklyClearly.buildHTMLForNode = function (_nodeToBuildHTMLFor, _custom_mode) {
        var _global__element_index = 0,
            _global__the_html = '',
            _global__exploreNodeToBuildHTMLFor = this.exploreNodeAndGetStuff(_nodeToBuildHTMLFor, true);
        ...
        var _recursive = function (_node) {
            ...
            // vars
            var _explored = false,
                _tag_name = FeweeklyClearly.getContent_detectNodeTagName(_node),
            ...
            case ('iframe'):
                ...
                FeweeklyClearly.log('Video SRC: ' + _src);
                ...
                if (_skip === false) {
                    ...
                    for (var i = 0, _i = keepStuffFromDomain__video.length; i < _i; i++) {
            ...
            // skipped link
            if (_tag_name === 'a') {
                _explored = (_explored || FeweeklyClearly.exploreNodeAndGetStuff(_node, true));
                switch (true) {
            ...
            if (parsingOptions._elements_link_density.indexOf('|' + _tag_name + '|') > -1) {
                ...
                case (FeweeklyClearly.language === 'cjk' && (_explored._length__plain_text > (65 * 3 * 1))):
            ...
            // start tag
            if (parsingOptions._elements_ignore_tag.indexOf('|' + _tag_name + '|') < 0) {
                ...
                if (_tag_name in parsingOptions._elements_keep_attributes) {
                    for (var i = 0, _i = parsingOptions._elements_keep_attributes[_tag_name].length; i < _i; i++) {
                        var _attribute_name = parsingOptions._elements_keep_attributes[_tag_name][i],
            ...
            // child nodes
            if (parsingOptions._elements_self_closing.indexOf('|' + _tag_name + '|') < 0) {
            ...
            if (_tag_name === 'img') {
                ...
                case (_explored._is__image_large):
                    FeweeklyClearly.images.push({src: FeweeklyClearly.getAbsoluteUrl(_node.src), width: _$node.width(), height: _$node.height()});
            ...
            if (_tag_name === 'a') {
                _explored = (_explored || FeweeklyClearly.exploreNodeAndGetStuff(_node, true));
                _$node = $(_node);
                FeweeklyClearly.links.push({href: FeweeklyClearly.getAbsoluteUrl(_node.href), text: _$node.text()});
                switch (true) {
            ...
            case (_tag_name === 'td'):
                break;
            default:
                _contents = _global__the_html.substr(_pos__start__after, (_pos__end__before - _pos__start__after));
                _contents = _contents.replace(/(<br \/>)/gi, '');
                _contents = _contents.replace(/(<hr \/>)/gi, '');
                _contentsLength = FeweeklyClearly.getTextLength(_contents);
                switch (true) {
            ...
            if (parsingOptions._elements_link_density.indexOf('|' + _tag_name + '|') > -1) {
                ...
                case (!((FeweeklyClearly.language === 'cjk') && (_contentsLength / _initialLength) < 0.1)):
                case ((_global__exploreNodeToBuildHTMLFor && ((_explored._length__plain_text / _global__exploreNodeToBuildHTMLFor._length__plain_text) > 0.25))):
                case ((FeweeklyClearly.language === 'cjk') && (_global__exploreNodeToBuildHTMLFor && ((_explored._length__plain_text / _global__exploreNodeToBuildHTMLFor._length__plain_text) > 0.1))):
                    break;
                default:
                    _global__the_html = _global__the_html.substr(0, _pos__start__before);
                    return;
    ...
    FeweeklyClearly.find = function () {
        // get content
        var _found = this.findInPage(this.window);
        ...
        (function () {
            while (true) {
                ...
                if (_prevNode.previousSibling) {
                    ...
                    // get html
                    var _h = FeweeklyClearly.buildHTMLForNode(_prevNode, 'above-the-target');
                    _prevHTML = _h + _prevHTML;
                    _foundHTML = _h + _foundHTML;

                    // finished?
                    if (FeweeklyClearly.getTextLength(_prevHTML.replace(/<[^>]+?>/gi, '')) > (65 * 3 * 3)) {
                        return;
                    }
        ...
        if (!_foundTitle) {
            if (document.title > '') {
                FeweeklyClearly.log('use title from document.title');
                var _the_title = '',
                    _doc_title = document.title,
                    _doc_title_parts = [],
    ...
    FeweeklyClearly.findInPage = function (_pageWindow) {
        var _firstCandidate = false,
            _secondCandidate = false,
            _targetCandidate = false;

        var _stuff = this.exploreNodeAndGetStuff(_pageWindow.document.body);

        this.log('processCandidates: 1st round');
        var _processedCandidates = this.processCandidates(_stuff._candidates);
        ...
        default:
            ...
            this.log('processCandidates: 2nd round');
            var _processedCandidatesSecond = this.processCandidatesSecond(_processedCandidates);
            ...
            // compute again
            this.log('processCandidates: 3rd round');
            _firstCandidate['__points_history_final'] = this.computePointsForCandidateThird(_firstCandidate, _firstCandidate);
            _firstCandidate['__points_final'] = _firstCandidate.__points_history_final[0];
            _secondCandidate['__points_history_final'] = this.computePointsForCandidateThird(_secondCandidate, _firstCandidate);
        ...
        this.log(_targetCandidate.__node);

        // get html
        var _html = this.buildHTMLForNode(_targetCandidate.__node, 'the-target');
    ...
    // auto-launch
    return FeweeklyClearly.getContent();
