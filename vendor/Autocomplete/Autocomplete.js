...
(function(window, document, undefined) {
	...
	AutocompleteProto.init = function() {
		...
		this.valuesShownWithNoEntry = this.options.srcCleanData;
	...
	AutocompleteProto.setElementFocus = function(event) {
		...
		if ((this.elementHasFocus = isFocusEvent)) {
			...
			// Show tags value if the input field gets the focus
			this.lastValue = -1;
	...
	AutocompleteProto.insertAfter = function(referenceNode, newNode) {
	...
	AutocompleteProto.matchValue = function(val) {
		// Get the value after the last comma
		var splittedVal = val.split(",");
		val = splittedVal[splittedVal.length-1].replace(/(^\s+|\s+$)/g,'');
		var escapeRgx, matchResult, matchRgx, matchText, results,
			matches = [], m = 0;

		if (!datalistSupported || !this.options.useNativeInterface) {
			if (val) {
				// Look if there is somethin in the cache
				...
				// No val provided show the cleanData
				results = this.valuesShownWithNoEntry.slice(0, 10);
			}
		...
		if (actionable[which]) {
			...
			// DOWN or UP key:
				...
				if ((shownValuesLen = this.shownValues.length)) {
					if ((which === 40 && this.highlightIdx === shownValuesLen - 1)) {
						// DOWN and last element
						this.setHighlightedIndex(0);
					} else if (which === 38 && this.highlightIdx === 0) {
						// UP and first element
	...
	AutocompleteProto.selectValue = function(event) {
		...
		// Select value:
		if (this.highlightIdx > -1) {
			var selectedValue = this.shownValues[this.highlightIdx];
			var splittedElementValue = this.element.value.split(",");

			// Remove whitespaces
			var splittedElementValueLength = splittedElementValue.length;
			for (var i = 0; i < splittedElementValueLength; i++) {
				splittedElementValue[i] = splittedElementValue[i].replace(/(^\s+|\s+$)/g,'');
			}

			// Result are values splitted by ", "
			if (splittedElementValue.length > 1) {
				splittedElementValue[splittedElementValue.length-1] = selectedValue;
				this.element.value = splittedElementValue.join(", ") + ", ";
			}
			else {
			...
			if (this.options.valueSelectedCallback !== undefined) {
				this.options.valueSelectedCallback();
			}
		}
